...
export class OrchestratorExecutorService {
  ...
  async execute(graph: OrchGraph, chains: Chain[]): Promise<void> {
    ...
    await Promise.all(
      reachableIds
        ...
        .map(id => getPromise(id))
    );

    // Mark any unreached idle nodes as skipped
    run.nodes.filter(n => n.status === 'idle').forEach(n => { n.status = 'skipped'; });
  ...
  private setStatus(run: OrchRun, nr: OrchNodeRun, status: NodeRunStatus): void {
    ...
    if (['success', 'failure', 'skipped'].includes(status)) nr.completedAt = new Date().toISOString();
    this.push(run);
  }
  ...
  private fetchLatestTag(fullName: string): Promise<string | null> {
    return new Promise(resolve =>
      this.gh.listTags(fullName).subscribe({ next: ts => resolve(ts[0]?.name ?? null), error: () => resolve(null) })
    );
  }
  ...
  private triggerStep(
    fullName: string, workflowId: number, ref: string, inputs: Record<string, string>
  ): Promise<string | null> {
    return new Promise(resolve =>
